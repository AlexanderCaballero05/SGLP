...
require './facturacion_mayor_db.php';
...
<br>
<h2 align="center" style="color:black;" ><b>EMISION DE ACTAS DE ENTREGA DE LOTERIA MAYOR</b></h2>
<br>
</section>
...
if (isset($_POST['seleccionar'])) {
	...
	$inventario_asignado = mysqli_query($conn, "SELECT COUNT(id_empresa) as conteo FROM sorteos_mezclas as a   WHERE a.id_sorteo = '$id_sorteo' AND  a.id_empresa = '$id_empresa' AND a.cod_factura IS NULL ");
	...
	$cantidad_asignada = $cantidad_asignada * 100;
	...
	if ($tipo_rebaja_e == 2) {
		...
		$comision = $valor_nominal * $porcentaje_comision;
	} else {
	...
	$consulta_facturas = mysqli_query($conn, "SELECT MAX(no_factura) as max_id FROM facturacion_mayor ");
...
<table class = 'table table-bordered'>
	...
	<tr>
		<td width = '10%'><input class = 'form form-control' style = 'width:100%' type = 'hidden' value="<?php echo date('Y-m-d'); ?>" name = 'fecha_expedicion' readonly>
		<?php echo date('Y-m-d'); ?>
		</td>

		<td width = '10%'><input class = 'form form-control' style = 'width:100%' type = 'hidden' name = 'no_factura' value = '<?php echo $factura; ?>' >
		<?php echo $factura ?>
		</td>
...
<?php
$facturaciones = mysqli_query($conn, " SELECT a.id ,a.receptor, a.fecha_expedicion, a.no_factura, a.id_sorteo, a.fecha_sorteo, a.cantidad, a.valor_nominal, a.descuento, a.rebaja_depositario, a.valor_neto, a.id_empresa,  b.usuario_ftp, b.clave_ftp FROM facturacion_mayor as a INNER JOIN empresas as b ON a.id_empresa = b.id WHERE a.id_sorteo = '$id_sorteo' AND a.estado = 'A' ");
	...
	while ($factura = mysqli_fetch_array($facturaciones)) {
		...
		echo "<td>
<a class='btn btn-primary' target='_blank' href= './print_acta_entrega_mayor.php?c=" . $factura['id'] . "'>
...
<span class = 'fa fa-times-circle'></span>
		...
		if ($factura['usuario_ftp'] != "") {

			$concat_ftp = $factura['id_sorteo'] . "$" . $factura['id_empresa'] . "$" . $factura['usuario_ftp'] . "$" . $factura['clave_ftp'] . "$" . $factura['no_factura'];

			echo "<td><button value = '" . $concat_ftp . "' type = 'submit' name = 'generar_ftp' class = 'btn btn-primary '> <span class = 'fa fa-file-alt'></span>
</button></td>";
		} else {
			echo "<td></td>";
		}
