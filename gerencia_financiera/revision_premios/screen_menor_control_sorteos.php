                            ...
                            while ($row_sorteos_vigentes = mysqli_fetch_array($query_sorteos_vigentes))  {
                              ...
                              $_fecha_vencimiento =  date("d-m-Y", strtotime($_fecha_vencimiento));
                                     ...
                                     $impto_acumulado_derecho = 0;   
                                                    ...
                                                    {  
                                                      ...
                                                      $query_pagado_derecho= mysqli_query($conn, "SELECT a.transactiondate  FROM menor_pagos_detalle a, menor_pagos_recibos b WHERE a.transactioncode= b.transactioncode and sorteo='$_sorteo' and numero='$_numero_ganador_derecho' and serie='$_numero_serie_ganador_derecho' and a.transactionstate=1 ");
                                                       ...
                                                       if (mysqli_num_rows($query_pagado_derecho)>0) {
                                                          ...
                                                          } else if (mysqli_num_rows($query_vendido_derecho)>0) { $estado_derecho='No Cobrada por el Cliente';
                                                          } else  { $estado_derecho='No vendida';  }
                                      ...
                                      $impto_acumulado_reves = 0;                              
                                      while ($_numeros_ganadores_reves=mysqli_fetch_array($query_numeros_ganadores_reves))
                                      {
                                             $_numero_ganador_reves=$_numeros_ganadores_reves['numero_premiado_menor'];
                                             $_monto_ganador_reves=$_numeros_ganadores_reves['pago_premio'];
                                             if ($_numero_ganador_reves==00 or $_numero_ganador_reves==11 or $_numero_ganador_reves==22 or $_numero_ganador_reves==33 or $_numero_ganador_reves==44 or $_numero_ganador_reves==55 or $_numero_ganador_reves==66 or $_numero_ganador_reves==77 or $_numero_ganador_reves==88 or $_numero_ganador_reves==99) 
                                             {
                                               $_monto_ganador_reves=1100;
                                             }
                                                    $query_series_ganadores_reves=mysqli_query($conn, "SELECT a.numero_premiado_menor, b.pago_premio FROM sorteos_menores_premios a, premios_menores b WHERE a.premios_menores_id=b.id and a.sorteos_menores_id=$_sorteo  AND  b.tipo_serie='REVES' and b.clasificacion='SERIE';");                                                    
                                                    $estado_reves='';   $impto_serie_pagar_reves=0;     $impto_reves=0;                                             
                                                    while ($_series_ganadores_reves=mysqli_fetch_array($query_series_ganadores_reves))
                                                    {  
                                                      $_numero_serie_ganador_reves = $_series_ganadores_reves['numero_premiado_menor'];
                                                      $_monto_serie_ganador_reves  = $_series_ganadores_reves['pago_premio'];
                                                      $suma_pago_reves             = $_monto_ganador_reves + $_monto_serie_ganador_reves;

                                                      if ($suma_pago_reves>30000) {  $impto_reves=$suma_pago_reves*0.10;  }  else  { $impto_reves=0; }

                                                      $fecha_pago_reves="";  
                                                      $neto_reves=$suma_pago_reves-$impto_reves;   
                                                      $query_pagado_reves= mysqli_query($conn, "SELECT a.transactiondate  FROM menor_pagos_detalle a, menor_pagos_recibos b WHERE a.transactioncode= b.transactioncode and sorteo=$_sorteo and numero=$_numero_ganador_reves and serie=$_numero_serie_ganador_reves and a.transactionstate=1 ");
                                                      $query_vendido_reves= mysqli_query($conn, "SELECT * FROM archivo_pagos_menor WHERE  sorteo='$_sorteo' and numero='$_numero_ganador_reves' and serie='$_numero_serie_ganador_reves'");
                                                      if (mysqli_num_rows($query_pagado_reves)>0) {
                                                        ...
                                                        $impto_serie_pagar_reves               = $impto_reves;
                                                        $impto_acumulado_reves                 = $impto_acumulado_reves+ $impto_reves;
                                                        
                                                      } else if (mysqli_num_rows($query_vendido_reves)>0) { $estado_reves='No Cobrada por el Cliente';
                                                      } else  { $estado_reves='No vendida';  }
                                                        echo " <tr><td align='center'>".$_sorteo."</td> 
                                                                   <td align='center'>".$_fecha_sorteo."</td> 
                                                                   <td align='center'>".$_fecha_vencimiento."</td> 
                                                                   <td align='center'>".$estado_reves."</td>
                                                                   <td align='center'>".$fecha_pago_reves."</td> 
                                                                   <td align='center'>".$_numero_ganador_reves."</td>
                                                                   <td align='center'>".$_numero_serie_ganador_reves."</td>
                                                                   <td align='right'>".number_format($suma_pago_reves,2,'.',',')."</td>
                                                                   <td align='right'>".number_format($impto_reves,2,'.',',')."</td>
                                                                   <td align='right'>".number_format($neto_reves,2,'.',',')."</td>
                                                                   <td align='center'></td></tr>"; 
