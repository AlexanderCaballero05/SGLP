...
<div id='div_wait'></div>
<section style="text-align: center; background-color:#ededed; padding-top: 10px;"><h3>Reporte de Asignación de Lotería Mayor</h3> <br></section>
        ...
        <div class="card card-body" id="non-printable" >
          ...
          <div class="input-group-prepend" style="margin-left: 5px;" ><span  class="input-group-text">Año de Remesa: </span></div>
                            <select name="slctanoremesa" id="slctanoremesa" class="form-control">
                             ...
                             <?php 
                               $result_ano_remesa=mysqli_query($conn, "SELECT ano_remesa FROM rp_asignacion_agencias_revisor_mayor group by ano_remesa order by ano_remesa desc; ");
            ...
            <div class="input-group-prepend" style="margin-left: 5px;" ><span  class="input-group-text">Remesa: </span></div>
                            <select name="slctremesa" id="slctremesa" class="form-control">
                             <option value='0'>Seleccione Uno</option>
                             <?php 
                               $result_remesa=mysqli_query($conn, " SELECT remesa FROM rp_asignacion_agencias_revisor_mayor group by remesa order by remesa desc; ");
...
if (isset($_POST['seleccionar'])) 
{
  ...
  {
    $_remesa=$_POST['slctremesa'];  $ano_remesa=$_POST['slctanoremesa']; 
    $query_fechas=mysqli_query($conn, "SELECT date(transactiondate) fecha FROM rp_asignacion_agencias_revisor_mayor where remesa=$_remesa and ano_remesa='$ano_remesa'group by date(transactiondate) order by fecha asc");

              echo "<div class='col-md-12'><hr><h3>Reporte de Lotería Mayor Entregada Remesa ".$_remesa." </h3>
                ...
                {   
                    $_fecha_pago= $row_fechas['fecha'];                     
                     echo "<tr class='info'><td colspan='6' align='center'><label><i class='far fa-calendar-alt'></i> &nbsp;  &nbsp;  ".$_fecha_pago."<span></label></td></tr>";

                    $query_agencias_fecha=mysqli_query($conn, "SELECT transactionagency, b.nombre seccional, c.nombre_completo usuario,  cant_decimos as cant_numeros, netopayment, usuario_revision, fecha_recepcion 
                                                               FROM rp_asignacion_agencias_revisor_mayor a, seccionales b, pani_usuarios c 
                                                               WHERE date(transactiondate)='$_fecha_pago' and remesa=$_remesa and ano_remesa='$ano_remesa' and a.usuario_revision=c.id and a.transactionagency=b.cod_seccional and b.id_empresa=3 order by fecha_recepcion asc");
                     ...
                     while ($row_info = mysqli_fetch_array($query_agencias_fecha))                 
                     {
                                           ...
                                           $contador++; $cantidad_acumualdo_fecha=$cantidad_acumualdo_fecha+$cantidad;  $neto_acumulado_fecha= $neto_acumulado_fecha+$neto;
                     }

                     echo     "<tr><td></td></tr>
                               <tr class='table-info'><td align='center' colspan='2'><label>Total por Fecha</label></td>                                                   
          ...
          echo "</tbody></table></div>                              
                <div class='row'>
                  ...
                  <div class='col-md-4'>
                    <a class='btn btn-success btn-lg' style='width:100%; background-color: #139949;' href='./_PDF_asignacion_revisores_mayor.php?remesa=".$_remesa."'  target='_blank' role='button'><span class='glyphicon glyphicon-save' aria-hidden='true'></span> Imprimir Reporte</a></div><div class='col-md-4'>
                  </div>
                </div>";     
  }
  ...
  {      
      ...
      echo "<div class='col-md-12'><hr><h3>Reporte de Lotería Mayor Entregada Remesa ".$_remesa." , Revisor ".$name_revisor." </h3>
                         <table id='tableinfo' class='table table-hover table-bordered table-sm'> 
                                 <thead><tr><td align='center'>No.</td>
      ...
      while ( $row_fecha= mysqli_fetch_array($query_fechas)) 
      {
         ...
         echo "<tr class='table-secondary'><td colspan='6' align='center'><i class='far fa-calendar-alt'></i> &nbsp;  &nbsp;  ".$_fecha_pp."</td></tr>";
                       ...
                       $query_info_revisor=mysqli_query($conn, "SELECT transactiondate, transactionagency, b.nombre, cant_decimos as cant_numeros, totalpayment, imptopayment, netopayment 
                                                         ...
                                                         WHERE a.transactionagency=b.cod_seccional and remesa=$_remesa and ano_remesa='$ano_remesa' and usuario_revision=$_revisor and transactiondate='$_fecha_pp' and b.id_empresa=3 order by fecha_recepcion asc");
                        ...
                        while ($row_info_revisor=mysqli_fetch_array($query_info_revisor)) 
                        {
                            ...
                            $fecha_recepcion=$row_info_revisor['transactiondate'];  $agencia=$row_info_revisor['transactionagency'];  $agencia_name=$row_info_revisor['nombre'];   $cantidad_numeros=$row_info_revisor['cant_numeros'];   $totalpayment=$row_info_revisor['totalpayment'];
                            $imptopayment=$row_info_revisor['imptopayment'];        $netopayment=$row_info_revisor['netopayment'];

                             echo "<tr><td align='center'>".$contador."</td>
                                                   <td align='left'>".$agencia." -- ".$agencia_name."</td>                         
                                                   <td align='center'>".number_format($cantidad_numeros)."</td>
                                                   ...
                                                   $contador++; $cantidad_acumualdo=$cantidad_acumualdo+$cantidad_numeros;  $total_acumulado= $total_acumulado+$totalpayment; $impto_acumulado= $impto_acumulado+$imptopayment; $neto_acumulado= $neto_acumulado+$netopayment;                                                   
                        }
                        echo        "<tr><td></td></tr>
                                              <tr class='table-success'><td align='center' colspan='2'><label>Total de la Remesa</label></td>                                                   
      ...
      echo        "<tr><td></td></tr>
                   <tr><td colspan='6' align='center'><label> Total General</label></td></tr>
 ...
 echo "</tbody></table></div>                              
                <div class='row'>
                    <div class='col-md-4'></div>
                    <div class='col-md-4'>
                      <a class='btn btn-success btn-lg' href='./_PDF_asignacion_revisores_mayor.php?remesa=".$_remesa."&revisor=".$_revisor."'  target='_blank' role='button'> Imprimir Reporte</a></div>
                    <div class='col-md-4'></div>
                </div>";     
  ...
  {
        $_revisor=$_POST['slctrevisor'];  echo $_revisor;
  }
  else { echo "<div class='alert alert-danger'> Debe Ingresar Parametros</div>";  }
