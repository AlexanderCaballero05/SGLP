    	       		...
    	       		<table  class="table table-hover table-sm table-bordered">
			    	    ...
			    	    if ($usuario_id=='1000') 
			    	    {
			    	     ...
			    	     $query_pagos_fecha=mysqli_query($conn, "SELECT date(a.transactiondate)  fecha_pago, b.transactionagency agencia_code, b.transactionagencyname agencia_name, count(numero) cantidad_pagada, sum(neto) neto_pagado ,
			    	       										 ...
			    	       										 WHERE a.remesa=$_remesa AND ano_remesa = '$year' and a.transactionstate in (1) group by date(a.transactiondate), b.transactionagency order by date(a.transactiondate) asc ");
			    	    }
			    	    else
			    	    {
			    	     	$query_pagos_fecha=mysqli_query($conn, "SELECT date(a.transactiondate)  fecha_pago, b.transactionagency agencia_code, b.transactionagencyname agencia_name, count(numero) cantidad_pagada, sum(neto) neto_pagado ,
			    	       										 ...
			    	       										 (  sum( case when (estado_revision =2 ) then neto else 0 end) ) 'neto_debitado'
			    	       										 FROM menor_pagos_detalle a INNER JOIN menor_pagos_recibos b ON a.transactioncode=b.transactioncode 
			    	       										 WHERE a.remesa=$_remesa AND ano_remesa = '$year'  and  a.transactionstate in (1) and usuario_revision=$usuario_id group by date(a.transactiondate), b.transactionagency order by date(a.transactiondate) asc ");
			    	      }		
