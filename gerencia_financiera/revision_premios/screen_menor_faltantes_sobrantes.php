...
<section style="text-align: center; background-color:#ededed; padding-top: 10px;"><h3>Faltantes y Sobrantes de Loteria Menor</h3> <br></section>
<section>
<a style = "width:100%" id="non-printable"  class="btn btn-secondary" role="button" data-toggle="collapse" href="#collapse1" aria-expanded="false" aria-controls="collapse"> SELECCIÓN DE PARAMETROS  <i class="far fa-hand-point-down fa-lg"></i></a>
                ...
                <td>                  
                     <div class="input-group" style="margin:10px 0px 10px 0px; width: 100%" align="center">
                        ...
                        <select name="txtremesa" id="txtremesa" required="true" class="form-control">
                            <option> Seleccione Uno </option>
                          <?php 
                          $query_agencia=mysqli_query($conn, "SELECT remesa from menor_pagos_detalle where ano_remesa='2019' group by remesa order by remesa DESC;");
                          while ($row_agencia=mysqli_fetch_array($query_agencia)) 
                          {
                ...
                <td>
                    <div class="input-group" style="margin:10px 0px 10px 0px; width: 100%" align="center">
                        ...
                        <select name="slctsorteo" id="slctsorteo" required="true" class="form-control">
                            <option> Seleccione Uno </option>
                          <?php 
                          $query_agencia=mysqli_query($conn, "SELECT sorteo from menor_pagos_detalle group by sorteo order by sorteo DESC;");
                          while ($row_agencia=mysqli_fetch_array($query_agencia)) 
                          {
 ...
 <hr><hr>
 <section>
<h3 align="center"><div class="alert alert-success"> <strong> Listado de Sobrantes y Faltantes de Lotería Menor </strong></div></h3>
  <hr>
    <div id="reporte" class="table-responive" align="center"> <br><br>
         <table id="table_id1" align="center" class="table table-hover table-bordered table-responsive">  
             ...
             <tbody>
             <?php
              $result = mysqli_query($conn, "SELECT id, remesa, sorteo, numero, serie, `registro`, `totalpayment`, `imptopayment`, `netopayment`, transactionagencyname, date(`transactiondate`) fecha, date(`creationdate`) fecha_creacion, `creationuser`, `registertype`, `coment` FROM `rp_faltantes_sobrantes_menor`  ");
...
if (isset($_POST['btnguardar'])) 
{
        ...
        $car_txt= "INSERT INTO rp_faltantes_sobrantes_menor(remesa, sorteo, numero, decimos, registro, totalpayment, imptopayment, netopayment,  transactionagencyname, transactiondate, registertype, creationuser,  coment) 
        VALUES ('$_remesa',  '$_sorteo', '$_numero', '$_serie', '$_registro', '$_total', '$_impto', '$_neto',  '$_agencia', '$_fecha_inicial', '$_tipo', '$user_name', '$_comment' ) ";

        echo $car_txt;

        if (mysqli_query($conn, "INSERT INTO rp_faltantes_sobrantes_menor(remesa, sorteo, numero, serie, registro, totalpayment, imptopayment, netopayment,  transactionagencyname, transactiondate, registertype, creationuser,  coment) 
        ...
        {
            $query_id=mysqli_query($conn, "SELECT LAST_INSERT_ID(id) id from rp_faltantes_sobrantes_menor order by id desc limit 1;");
  ...
  ?>
            ...
            .then((willDelete) => {
              if (willDelete) {
                 window.open('./_PDF_rp_add_faltantes_sobrantes_menor.php', '_blank');                 
              }  
            });
            </script>
        <?php 
        } else {
          echo '<div class="alert alert-danger" role="alert"> Error inesperado  '.mysqli_error().', favor vuelva a intentarlo</div>';
        }
