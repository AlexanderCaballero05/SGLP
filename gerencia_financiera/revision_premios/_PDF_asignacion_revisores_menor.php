...
$ano_remesa= $_GET['year'];
$time=mysqli_query($conn, "SELECT CURRENT_TIMESTAMP() fecha_hora;");
$time_object=mysqli_fetch_object($time);
...
echo "<p align='left' style='font-size: 10'  ><label>RP_LM ".$fecha_hora."</label></p>";
...
echo '<table align="center">
        ...
        <tr><td width="20%"></td>
           <td  width="60%"  style="font-family: Arial; font-size: 10pt;"> <div align="center"><label style=" font-family: Arial; font-size:10pt;" >Patronato Nacional de la Infancia PANI <br> Departamento de Revision de Premios<br>Remesa No.   '. $_remesa .' asignada a: <br> '.$name_revisor.'  </label></div></td>
           <td width="20%"></td>
        </tr>      
     </table><br>';
  ...
  while ( $row_user_name =mysqli_fetch_array($query_revisor_name)) { $name_revisor=  $row_user_name['nombre_completo'];   }
       ...
       echo "<div class='col-md-12'>
                         <table id='tableinfo' align='center' style='font-size: 10' class='table table-hover table-bordered'> 
       ...
       $query_fechas=mysqli_query($conn, "SELECT date(transactiondate) fecha FROM rp_asignacion_agencias_revisor_menor where remesa=$_remesa and usuario_revision=$_revisor and ano_remesa in ('$ano_remesa') and date(transactiondate)>'2019-06-01' group by date(transactiondate) order by fecha asc");
       while ( $row_fecha= mysqli_fetch_array($query_fechas)) 
       {
        ...
        echo "<tr class='info'><td colspan='6' align='center'><label> <span class='glyphicon glyphicon-calendar'> &nbsp;  ".$_fecha_pp."<span></label></td></tr>";
                       ...
                       $query_info_revisor=mysqli_query($conn, "SELECT transactiondate, transactionagency, b.nombre, cant_numeros, totalpayment, imptopayment, netopayment 
                                                        ...
                                                        WHERE a.transactionagency=b.cod_seccional and b.id_empresa=3 and remesa=$_remesa and usuario_revision=$_revisor and transactiondate='$_fecha_pp'  order by fecha_recepcion asc");
                        ...
                        echo        "<tr><td></td></tr>
                                              <tr class='success'><td align='center' colspan='2'><label>Total de la Remesa</label></td>                                                   
