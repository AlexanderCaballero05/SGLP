...
<div id='div_wait'></div>
<section style="text-align: center; background-color:#ededed; padding-top: 10px;"><h3>Detalle de Pagos Pendientes de Revision de Lotería Mayor de Fecha <?php echo $fecha; ?>  </h3>   <br></section>
 <?php    
    $cantidad_acumulado_final=0;  $neto_acumulado_final=0;      
         ...
         echo  "<table width='96%'  id='tableinfo' align='center' class='table table-hover table-sm table-bordered'>         
          ...
          $query_numeros=mysqli_query($conn, "SELECT b.transactionagency,  b.transactionagencyname, b.transactionusername, a.sorteo, a.numero, a.decimos, a.registro, a.totalpayment, a.imptopayment, a.netopayment,  a.fecha_recepcion_banco, a.remesa, c.usuario_revision, d.nombre_completo
                                              FROM mayor_pagos_detalle a , mayor_pagos_recibos b, rp_asignacion_agencias_revisor_mayor c, pani_usuarios d
                                              WHERE 
        ...
        while ($row_numeros=mysqli_fetch_array($query_numeros)) 
        {
              ...
              $serie                       = $row_numeros['decimos'];
              $registro                    = $row_numeros['registro'];
              $principal                   = $row_numeros['totalpayment'];
              $impto                       = $row_numeros['imptopayment'];
              $neto                        = $row_numeros['netopayment'];
