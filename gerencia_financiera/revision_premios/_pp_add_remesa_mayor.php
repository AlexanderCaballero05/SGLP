...
$_remesa=$_GET['remesa'];  $_agency=$_GET['agencia'];   $_fecha_pago=$_GET['fecha_pago'];   $_action=$_GET['acc']; $_ano_remesa=$_GET['ano_remesa'];

  if ($_action==1)   
  { 
    $requete = mysqli_query($conn, "UPDATE mayor_pagos_detalle a, mayor_pagos_recibos b set remesa=$_remesa , ano_remesa=$_ano_remesa,  fecha_recepcion_banco=CURRENT_TIMESTAMP  where a.transactioncode=b.transactioncode and date(a.transactiondate) = '$_fecha_pago'  and b.transactionagency='$_agency' and a.transactionstate in (1,3) ");  
   ...
   if ($requete_consulta_agencia==false) {  echo mysqli_error($conn);  } else { echo "bien la consulta "; }
    while ($row_agencias_info= mysqli_fetch_array($requete_consulta_agencia) ) 
    {
       ...
       $requete_insert = mysqli_query($conn, "INSERT INTO rp_asignacion_agencias_revisor_mayor(transactionagency, transactiondate, cant_decimos, totalpayment, imptopayment, netopayment, remesa, ano_remesa) 
        VALUES($_agency, '$_fecha_pago', $cantidad_ag, $monto_ag, $impto_ag, $neto_ag, $_remesa,  $_ano_remesa ) ");  
         if ($requete_insert==false) {  echo mysqli_error($conn);  } else { echo "bien el insert "; }
    }   
  }
  else if ($_action==2)    
  { 
    $requete = mysqli_query($conn, "UPDATE mayor_pagos_detalle a, mayor_pagos_recibos b SET remesa=0, ano_remesa=null, fecha_recepcion_banco=null WHERE a.transactioncode=b.transactioncode and date(a.transactiondate) = '$_fecha_pago'  and b.transactionagency=$_agency and a.transactionstate IN (1,3) ");  
  ...
  {    
              $_agencia=$_agency;
              echo "<div class='alert alert-info' align='center'> <label style='font-family: Arial; font-size: 12pt;'>Información de Billetes Pagados en fecha ".$_fecha_pago." con número de remesa No. ".$_remesa.". del año ".$_ano_remesa.".</label></div>";
 ...
 echo "</table></div></div><div class='col-md-1' align='center'></div></div>";
                    ...
                    echo  "<div class='row'>                             
                        <div class='col-md-1' align='center'></div>
                          <div class='col-md-10' align='center'><legend><div class='alert alert-success'>Información Seleccionada para la Remesa No. ".$_remesa."  del año ".$_ano_remesa."</div></legend> 
                                  <div class='table-responsive' style='padding-bottom: 0px; page-break-after: always;'>
                                ...
                                if (mysqli_num_rows($result_remesados)>0)
                                  {
                                    ...
                                    {  
                                      ...
                                      echo "<tr><td  align='left'>".$row_remesados['transactionagency']." -- ".$row_remesados['seccional']."</td>
                                                ...
                                                <td  align='center'  id='btn-rev'>  
                                                <button type='button' name='option1".$contador_remesados."' id='option1".$contador_remesados."' onclick='add_remesa(this.value, 2)'  value='".$_remesa."--".$row_remesados['transactionagency']."--".$row_remesados['seccional']."--".$_fecha_pago."--".$_ano_remesa."' class='btn btn-sm btn-danger'><i class='far fa-thumbs-down fa-lg'></i>  Quitar a la Remesa</button>       
                                                </td></tr></tbody>";
                                                $contador_remesados ++;    
                                     }                                      
  ...
  {
     echo "<div class='alert alert-danger'><label>".mysqli_error($conn)." en el update</label></div>";  
  }       
