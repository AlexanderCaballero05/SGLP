...
<div id='div_wait'></div>
<section style="text-align: center; background-color:#ededed; padding-top: 10px;"><h3>Ajustes de Lotería Menor Fecha # <?php echo $fecha; ?>  </h3><br></section>
...
<tbody>
<?php 
 $query= mysqli_query($conn, "SELECT id, transactiondate, remesa, transactionagencyname, transactionusername, comentario_revisor comentario, tipo_documento, incidencia, fecha_creacion, sorteo, numero, serie, registro, neto FROM `rp_notas_credito_debito_menor` where date(transactiondate)='$fecha'");
 ...
 {    
    while ($row_query= mysqli_fetch_array($query)) 
    {
       echo "<tr ><td>".$row_query['id']."</td>
                  ...
                  <td align='center'>".$row_query['serie']."</td>
