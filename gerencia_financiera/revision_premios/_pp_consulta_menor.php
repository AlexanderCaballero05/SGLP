...
$query_caduco=mysqli_query($conn, "SELECT  (45 - DATEDIFF(CURRENT_DATE, fecha_sorteo)) dias FROM sorteos_menores where id=$sorteo");
...
$query_pagado=mysqli_query($conn,"SELECT b.transactioncode recibo , a.transactioncore, b.transactionagencyname agencia , b.transactionusername cajero, b.transactiondate fecha, b.transactionwinnername ganador, a.principal totalpayment , a.impto imptopayment , a.neto netopayment, a.registro, a.estado_revision, a.usuario_revision_name, a.fecha_revision, a.remesa FROM menor_pagos_detalle a, menor_pagos_recibos b where a.transactioncode=b.transactioncode and a.sorteo=$sorteo and a.numero=$numero and serie=$serie and a.transactionstate=1 ;");
...
else if (mysqli_num_rows($query_pagado)>0)
{
   ...
   $html_pagado_totalidad .= '<tr class="table-success">
                                        ...
                                        <td colspan="2    ">Remesa(s): '.$num_remesa.'</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div></div></div>';
    ...
    if ($query === false) {  echo mysqli_error();  }
      ...
      {        
        ...
        if ($estado==1) 
        {
         ...
         echo "<div class='card'>
         <div class='card-header bg-success text-white' >El Billete numero <strong>".$numero."</strong> con serie <strong>".$serie."</strong> del sorteo <strong>".$sorteo."</strong>  tiene premio.<strong class='text-warning'>-- ".$estado_premio_caduco."</strong></div>
