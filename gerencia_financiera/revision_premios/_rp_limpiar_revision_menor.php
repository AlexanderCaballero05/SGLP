<?php
	 ...
	 while ($row_usu=mysqli_fetch_array($query_usuario) )
	 {
	 	 $nombre_completo=$row_usu["nombre_completo"];
	   	...
	   	$requete = mysqli_query($conn, "UPDATE menor_pagos_detalle SET estado_revision=null , usuario_revision=$_user, usuario_revision_name='$nombre_completo',  fecha_revision=current_timestamp() WHERE id=$_id "); 
	   if ($requete) 
	   {
