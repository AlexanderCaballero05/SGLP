...
<section style="text-align: center; background-color:#ededed; padding-top: 10px;"><h3>Monitoreo de Sorteos de Lotería Mayor </h3> <br></section>
...
<?php 
	$sorteo_inicial=$_POST['sorteo_inicial'];     $sorteo_final=$_POST['sorteo_final']; 
    	...
    	<div class="table-responsive" id="tabla">
    	    <table id='tabla'  class="table table-hover table-sm table-bordered">
    	    	<thead><tr align="center">
		    	       				...
		    	       				<th id="no_print"></th>
		    	       				<th colspan="2">Pendiente || Caduco</th>		    	       				
		    	       			</tr> 
		    	       			<tr><th>Sorteo</th>
		    	       				<th>Fecha Sorteo</th>
								...
								$acumulado_billetes_pendientes    			= 0;        $acumulado_neto_pendiente       	   = 0;
			    	  			...
			    	  			while ($sorteo_inicial <= $sorteo_final) 
			    	  			{
									...
									$dias        			   = $info_sorteo->dias;
			    	  				...
			    	  				$query_compromiso=mysqli_query($conn, "SELECT (COUNT(*)*10) conteo, sum(neto) neto_compromiso from archivo_pagos_mayor where sorteo=$sorteo_inicial");
									...
									$query_pagado=mysqli_query($conn, "SELECT sum(decimos) conteo_pagado, sum(a.netopayment) neto_pagado from mayor_pagos_detalle a, mayor_pagos_recibos b where a.transactioncode = b.transactioncode and sorteo=$sorteo_inicial and a.transactionstate in (1,3) and tipo_premio<> 'E'");

			    	  				$info_pagado        = mysqli_fetch_object($query_pagado);
									$billetes_pagado    = $info_pagado->conteo_pagado;
									$neto_pagado        = $info_pagado->neto_pagado;

									$query_pagado_especies=mysqli_query($conn, "SELECT count(decimos) conteo_pagado, sum(a.netopayment) neto_pagado from mayor_pagos_detalle a, mayor_pagos_recibos b where a.transactioncode = b.transactioncode and sorteo=$sorteo_inicial and a.transactionstate in (1,3) and tipo_premio= 'E'");

									if (mysqli_num_rows($query_pagado_especies)>0) 
									{
										$info_pagado_especies        = mysqli_fetch_object($query_pagado_especies);
										$billetes_pagado_especies    = $info_pagado_especies->conteo_pagado;
										$neto_pagado_especies        = $info_pagado_especies->neto_pagado;
									}
									else
									{
										$billetes_pagado_especies    = 0;
										$neto_pagado_especies        = 0;	
									}
			    	  				 
			    	  				 $billetes_pagado_total = $billetes_pagado + $billetes_pagado_especies;
			    	  				 $neto_pagado_total     = $neto_pagado     + $neto_pagado_especies;
									...
									$query_revisado=mysqli_query($conn, "SELECT sum(decimos) conteo_revisado, sum(a.netopayment) neto_revisado from mayor_pagos_detalle a, mayor_pagos_recibos b where a.transactioncode = b.transactioncode and sorteo=$sorteo_inicial and a.transactionstate in (1,3) and estado_revision=1 and tipo_premio<> 'E'");
									...
									$query_revisado_especies=mysqli_query($conn, "SELECT count(decimos) conteo_revisado, sum(a.netopayment) neto_revisado from mayor_pagos_detalle a, mayor_pagos_recibos b where a.transactioncode = b.transactioncode and sorteo=$sorteo_inicial and a.transactionstate in (1,3) and estado_revision=1 and tipo_premio ='E'");

									if (mysqli_num_rows($query_revisado_especies)) 
									{
										$info_revisado_especies         = mysqli_fetch_object($query_revisado_especies);
										$billetes_revisados_especies    = $info_revisado_especies->conteo_revisado;
										$neto_revisado_especies         = $info_revisado_especies->neto_revisado;									   
									}
									else
									{
										$billetes_revisados_especies    = 0;
										$neto_revisado_especies         = 0;	
									}

			    	  				 
									$billetes_revisados_total    = $billetes_revisados + $billetes_revisados_especies ;
									$neto_revisado_total         = $neto_revisado   + $neto_revisado_especies ;

			    				 
									$query_revisado_debitos=mysqli_query($conn, "SELECT sum(decimos_nota) conteo_debitos, sum(neto_nota) neto_debitos FROM rp_notas_credito_debito_mayor WHERE sorteo=$sorteo_inicial");
									...
									$billetes_pendientes=$billetes_compromiso-$billetes_revisados-$billetes_debitos;
									$neto_pendiente=$neto_compromiso-$neto_revisado-$neto_debito;
			    	  				  ...
			    	  				  echo "<tr><td align='center'>".$sorteo_inicial."</td> 
			    	  				  			...
			    	  				  			<td align='center'>".$dias."</td> 
					    	       			  	...
					    	       			  	<td align='right'>".number_format($neto_compromiso,2)."</td> 
					    	       			  	 <td  id='no_print' >
							    	       		  	 <a role='button' target='_blank' href='./screen_mayor_obligacion_sorteo_detalle.php?sorteo=".$sorteo_inicial."' class='btn btn-primary btn-sm text-white'> <i class='far fa-eye'></i>  Ver  <a>
							    	       		 </td>
					    	       			  	<td align='center'>".$billetes_pagado_total."</td> 
					    	       			  	<td align='right'>".number_format($neto_pagado_total,2)."</td> 
					    	       			  	<td id='no_print'>
							    	       		  	 <a role='button' target='_blank' href='./screen_mayor_pagado_sorteo_detalle.php?sorteo=".$sorteo_inicial."' class='btn btn-primary btn-sm text-white'> <i class='far fa-eye'></i>  Ver  <a>
							    	       		 </td>
					    	       			  	...
					    	       			  	<td align='center'>
					    	       			  	  <a role='button' target='_blank' href='./_rp_mayor_pendiente_recepcionar_sorteo.php?sorteo=".$sorteo_inicial."' class='btn btn-primary btn-sm text-white'> <i class='far fa-eye'></i>  Ver  <a>
					    	       			  	</td>
					    	       			  	<td align='center'>".$billetes_revisados_total."</td> 
					    	       			  	<td align='right'>".number_format($neto_revisado_total,2)."</td> 
					    	       			  	 <td id='no_print'>
							    	       		  	 <a role='button' target='_blank' href='./screen_mayor_liquidacion_sorteo_detalle.php?sorteo=".$sorteo_inicial."' class='btn btn-primary btn-sm text-white'> <i class='far fa-eye'></i>  Ver  <a>
							    	       		 </td>
					    	       			  	<td align='center'>".$billetes_debitos."</td> 
					    	       			  	<td align='right'>".number_format($neto_debito,2)."</td> 
					    	       			  	<td id='no_print'>
							    	       		  	 <a role='button' target='_blank' href='./screen_mayor_notas_sorteo_detalle.php?sorteo=".$sorteo_inicial."' class='btn btn-primary btn-sm text-white'> <i class='far fa-eye'></i>  Ver  <a>
							    	       		 </td>
			    	 			...
			    	 			echo "<tr class='table-success'><td colspan='5' align='center'> Totales</td>
