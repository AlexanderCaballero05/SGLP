...
if (mysqli_num_rows($query_caduco)>0) {
  ...
  if ($dias_caduco<0) {
    $estado_premio_caduco_mayor="Premio no fue cobrado";
  }else{
    $estado_premio_caduco_mayor="";
  }
}
...
$query_pagado=mysqli_query($conn, "SELECT b.transactioncode recibo,  a.decimos, a.transactioncore, b.transactionagencyname agencia , b.transactionusername cajero, b.transactiondate fecha, b.transactionwinnername ganador, a.totalpayment , a.imptopayment , a.netopayment, a.registro, a.remesa,  estado_revision, usuario_revision, usuario_revision_name, fecha_revision , tipo_premio FROM mayor_pagos_detalle a, mayor_pagos_recibos b where a.transactioncode=b.transactioncode and a.sorteo=$sorteo and a.numero=$numero and a.transactionstate=1 ;");
   ...
   while ($_row_alert=mysqli_fetch_array($query_pagado))
   {
    ...
    if ($_estado_revision > 0 or !empty($_estado_revision) ) {
        $html_pagado_totalidad .= '<tr class="table-secondary">
                                        ...
                                        <td align="right">Remesa(s): '.$num_remesa.'</td> 
                                    </tr>';
    }
    else
    {
    	...
    	$query=mysqli_query($conn, " SELECT  token, detalle_venta, registro, total, impto, neto, tipo_pago  FROM archivo_pagos_mayor WHERE sorteo=$sorteo and numero=$numero");
          if ($query === false) {  echo mysqli_error($conn);  }
            ...
            $html_pagado_parcial .= ' <div class="card">
              ...
              <strong>El billete '.$numero.' del sorteo '.$sorteo.' tiene decimos pendientes de pago! <strong class="text-warning">-- '.$estado_premio_caduco_mayor.'</strong></strong>
              </div>
              <div class="card-body">'; 

            $query_historico_numero=mysqli_query($conn ,"SELECT a.transactiondate, a.transactioncode, b.transactionagencyname, b.transactionusername, a.decimos, a.totalpayment total, a.imptopayment impto, a.netopayment neto, a.estado_revision , a.usuario_revision_name, a.fecha_revision, a.remesa  FROM mayor_pagos_detalle a, mayor_pagos_recibos b WHERE a.transactioncode=b.transactioncode and a.sorteo=$sorteo and a.numero=$numero and a.transactionstate=1 ");
            if (mysqli_num_rows($query_historico_numero)>0) 
            {
                  ...
                  <table class="table table-hover table-sm table-bordered">                             
                        <thead align="center">
                          <tr class="table-secondary">
                            <th colspan="11">DECIMOS PAGADOS</th> 
                            </tr>
                            <tr class="table-secondary">
                            <th>Fecha de pago</th>
                            <th>Factura</th>
              ...
              while ($row_historico_pagado=mysqli_fetch_array($query_historico_numero)) 
              {
                 ...
                 $num_remesa                        = $row_historico_pagado['remesa'];
    ...
    if ($query === false) {  echo mysqli_error($conn);  }
      ...
      {     
        ...
        if ($estado==1 or $estado==3) 
        {
         ...
         echo "<div class='card'>
         <div class='card-header bg-success text-white' >El billete número ".$numero." del sorteo ".$sorteo." está premiado <strong class='text-warning'>-- ".$estado_premio_caduco_mayor."</strong></div>
         <div class='card-body'  align='center'            name='info_sorteo'  style='padding-top:2px;' id='info_sorteo'>
       ...
       echo "<div class='card'>
        ...
        $consulta_distribucion = mysqli_query($conn, "SELECT id_empresa FROM sorteos_mezclas as a INNER JOIN sorteos_mezclas_rangos as b ON a.num_mezcla = b.num_mezcla AND a.id_sorteo = b.id_sorteo WHERE a.id_sorteo = '$sorteo' AND b.rango < '$numero' ORDER BY b.rango DESC LIMIT 1  ");


        $ob_distribucion = mysqli_fetch_object($consulta_distribucion);
        $id_empresa = $ob_distribucion->id_empresa;

        if ($id_empresa == '' OR $id_empresa == NULL ) {
          
          $nombre_empresa = "";
          $message_distribucion  = "El billete no fue distribuido";

        }else{

          $c_empresas = mysqli_query($conn, "SELECT * FROM empresas WHERE id = $id_empresa ");
          $ob_empresa = mysqli_fetch_object($c_empresas);
          $nombre_empresa = $ob_empresa->nombre_empresa;  
          $message_distribucion  = "El billete fue distribuido a la Entidad ". $nombre_empresa ;
         ...
         <div class='card-body' name='info_sorteo1' id='info_sorteo1'>
                  ...
                  <div class='alert alert-danger'> ".$message_distribucion."</div>
         </div></div>";
