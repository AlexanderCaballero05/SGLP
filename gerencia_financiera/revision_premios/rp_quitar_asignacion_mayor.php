...
if (mysqli_query($conn, "UPDATE rp_asignacion_agencias_revisor_mayor SET usuario_revision=null  WHERE  date(transactiondate)='$fecha' AND remesa=$remesa ")==true) 
{
    $_remesa=$remesa; 
    ...
    if ($est==1) 
    {
          $query_agencias_fecha=mysqli_query($conn, "SELECT date(transactiondate) fecha, count(transactionagency) agencia, sum(cant_decimos) sum_numeros, sum(totalpayment) total, sum(imptopayment) impto, sum(netopayment)  neto
                                                     ...
                                                     WHERE remesa=$_remesa and ano_remesa='2019' GROUP BY fecha ORDER BY fecha_recepcion ASC");
    }
    else
    {
          $query_agencias_fecha=mysqli_query($conn, "SELECT date(transactiondate) fecha, count(transactionagency) agencia, sum(cant_numeros) sum_numeros, sum(totalpayment) total, sum(imptopayment) impto, sum(netopayment)  neto
                                                     FROM rp_asignacion_agencias_revisor_mayor a 
                                                     WHERE remesa=$_remesa AND ano_remesa='2019' and date(transactiondate)='$fecha' GROUP BY fecha ORDER BY fecha_recepcion ASC");
    }  
