...
$fecha=$_GET['fecha']; $remesa=$_GET['remesa'];    $est=$_GET['est'];  if (isset($_GET['ag'])) { $agencia_cod=$_GET['ag']; }

if (mysqli_query($conn, "UPDATE rp_asignacion_agencias_revisor_menor set usuario_revision=null  WHERE  date(transactiondate)='$fecha' and remesa=$remesa  and transactionagency=$agencia_cod  ")==true) 
{
    $_remesa=$remesa;   
          $query_agencias_fecha=mysqli_query($conn, "SELECT  id, transactionagencyname nombre,  transactionagency, cant_numeros, totalpayment total, imptopayment impto, netopayment neto,  IF(ISNULL(usuario_revision), 0, usuario_revision) usuario_revision
