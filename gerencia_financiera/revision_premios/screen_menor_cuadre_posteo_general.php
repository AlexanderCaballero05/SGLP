        ...
        <div class="card card-body" id="non-printable" >
          <div class="input-group" style="margin:10px 0px 10px 0px; width: 100%" align="center">
            <div class = "input-group-prepend"><span  class="input-group-text"> <i class="far fa-calendar-alt"></i> &nbsp;  Remesa: </span></div>
             ...
             <select class="form-control" id="id_usuario" name="slctrevisor"  required="true">
             <option value="1000"> Seleccionar ... </option>
              <?php 
              $query_usuarios=mysqli_query($conn, "SELECT usuario_revision, usuario_revision_name FROM menor_pagos_detalle WHERE ano_remesa='2021'  GROUP BY usuario_revision_name order by remesa desc");
...
if (isset($_POST['seleccionar'])) 
{
   $cantidad_acumulado_final=0;    $neto_acumulado_final=0;
    if ($_POST['slctremesa']>0 && $_POST['slctrevisor']>0 )
      ...
      echo "<div  class='alert alert-success' align='center'><h4> Pago de Premios correspondientes a la Remesa No. ".$_remesa."</h4></div>";

      $query_sorteos=mysqli_query($conn, "SELECT  sorteo FROM menor_pagos_detalle where estado_revision=1 and remesa=$_remesa  and usuario_revision=$_revisor and ano_remesa = $s_year and date(transactiondate)>='2021-10-15' group by  sorteo;");
      ...
      {   
        ...
        while($row_series=mysqli_fetch_array($query_series)) { $array_series[] = $row_series['numero_premiado_menor']; }
                                    ...
                                    a.sorteo=$sorteo and
                                    a.ano_remesa = $s_year and date(a.transactiondate)>='2021-10-12' and 
                                    ...
                                    a.estado_revision in (1,3)  and
                                    a.remesa=$_remesa and
                                    a.ano_remesa = $s_year and date(a.transactiondate)>='2021-10-15' and 
                                    a.usuario_revision=$_revisor and
                                    a.sorteo=$sorteo and
        ...
        echo "<tr><td colspan='2' align='center'><label>Gran Total</label></td>
         ...
         echo "<div align='center'> <a class='btn btn-success'  href='_PDF_cuadre_remesa_revisor_menor.php?remesa=".$_remesa."&revisor=".$_revisor."&ano_remesa=".$s_year."'  target='_blank' role='button'>
         <i class='far fa-save'></i> Imprimir Liquidación</a></div></div>";
