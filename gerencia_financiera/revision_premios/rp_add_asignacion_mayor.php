...
if ($est==1) {  $query_update=mysqli_query($conn, "UPDATE rp_asignacion_agencias_revisor_mayor set usuario_revision=$usuario_revision  WHERE  date(transactiondate)='$fecha' and remesa=$remesa ");  }
else         {  $query_update=mysql_query("UPDATE rp_asignacion_agencias_revisor_mayor set usuario_revision=$usuario_revision  WHERE  date(transactiondate)='$fecha' and remesa=$remesa and transactionagency=$agencia_cod ");  }

if ($query_update==true) 
{

$_remesa=$remesa;
  echo "<div class='alert alert-success' role='alert'><h3>Asignación de Loteria Mayor Remesa ".$_remesa."</h3></div><hr>
    ...
    if ($est==1) 
    {
        $query_agencias_fecha=mysqli_query($conn, "SELECT date(transactiondate) fecha,  count(transactionagency) agencia, sum(cant_decimos) sum_numeros, sum(totalpayment) total, sum(imptopayment) impto, sum(netopayment)  neto
                                                   FROM rp_asignacion_agencias_revisor_mayor a
