<?php
	 ...
	 if ($_estado==2)  
	 { 
	   	$requete = mysqli_query($conn, "UPDATE mayor_pagos_detalle SET estado_revision=2 , usuario_revision=$_user, usuario_revision_name='$nombre_completo',  fecha_revision=current_timestamp() WHERE id=$_id "); 
	   if ($requete) 
	   {
	 ...
	 { 
		...
		//$requete_txt = "UPDATE mayor_pagos_detalle SET estado_revision=1 , usuario_revision=$_user, usuario_revision_name='$nombre_completo', fecha_revision=current_timestamp() WHERE id=$_id "; 
		//echo "ddd  ". $requete_txt;
	 	$requete = mysqli_query($conn, "UPDATE mayor_pagos_detalle SET estado_revision=1 , usuario_revision=$_user, usuario_revision_name='$nombre_completo', fecha_revision=current_timestamp() WHERE id=$_id "); 
	  if ($requete) 
	   {
