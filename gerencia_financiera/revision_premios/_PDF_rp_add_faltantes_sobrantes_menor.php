...
$factura =  mysqli_query($conn, "SELECT remesa, sorteo, numero, serie, registro, totalpayment, imptopayment, netopayment, transactionagencyname, transactionusername, creationdate, creationuser, registertype, coment
from rp_faltantes_sobrantes_menor
...
<td width="20%"></td>
  ...
  <div align="center">
  <label>
          Patronato Nacional de la Infancia PANI <br> Departamento de Revisión de Premios<br>Reporte de  <?php echo $registertype ?>, de Lotería Menor <br> Del  <?php echo $creationdate; ?> <br> Perteneciente al Sorteo No. <?php echo $sorteo ?> <br> De la Remesa No. <?php echo $remesa ?>
  </label>
  </div>
  </td>
  <td width="20%"></td>
...
?>
   ...
   <table tyle='font-family: Arial; font-size: 12pt;' align="center" id="table_" align="center" class="table table-hover table-bordered table-sm">  
          <thead>
            <tr>
         ...
         <?php
            ...
            $result = mysqli_query($conn, " SELECT  `remesa`, `sorteo`, `numero`, serie, `registro`, totalpayment, imptopayment, netopayment, `transactionusername`, `transactionagencyname`, date(`transactiondate`) fecha, date(`creationdate`) fecha_creacion, `creationuser`, `registertype`, `coment` FROM `rp_faltantes_sobrantes_menor` where id=$cod_factura");
