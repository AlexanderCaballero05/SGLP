...
<br>
<h3  align="center" style="color:black; "  >REPORTE DE DISTRIBUCIONES DE LOTERIA MENOR BOLSAS (AGENCIAS)</h3>
<br>
</section>
...
if (isset($_POST['seleccionar'])) {
	...
	if (mysqli_num_rows($sorteos_en_fecha) > 0) {
		...
		$agencias = mysqli_query($conn, "SELECT DISTINCT(id_seccional) as id_seccional, nombre_seccional, CONCAT(departamento,'/',municipio) as depto_muni  FROM distribucion_menor_bolsas_banco WHERE id_sorteo BETWEEN '$min_sorteo' AND '$max_sorteo' GROUP BY id_seccional ORDER BY  id_seccional ASC ");
		...
		$distribuciones_sorteos = mysqli_query($conn, "SELECT id_sorteo ,id_seccional ,SUM(serie_final - serie_inicial + 1) as cantidad  FROM distribucion_menor_bolsas_banco WHERE id_sorteo BETWEEN '$min_sorteo' AND '$max_sorteo' GROUP BY id_sorteo , id_seccional ORDER BY id_sorteo, id_seccional ASC ");
...
if (isset($_POST['generar_excel'])) {
	...
	$objPHPExcel->getActiveSheet()->SetCellValue('A1', 'REPORTE DE DISTRIBUCION DE LOTERIA MENOR BOLSAS (AGENCIAS) ');
	...
	foreach ($m_agencias as $agencia) {
		...
		$objPHPExcel->getActiveSheet()->SetCellValue('B' . $row, $cod_agencia);
		$objPHPExcel->getActiveSheet()->SetCellValue('C' . $row, $nombre_agencia);
		$objPHPExcel->getActiveSheet()->SetCellValue('A' . $row, $depto_muni);
