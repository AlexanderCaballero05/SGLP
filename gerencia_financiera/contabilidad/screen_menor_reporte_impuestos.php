...
{ 
  ...
  $query_sorteos_mes=mysqli_query($conn, "SELECT sorteo as id, c.fecha_sorteo FROM menor_pagos_detalle a, menor_pagos_recibos b, sorteos_menores c WHERE a.transactioncode = b.transactioncode AND a.sorteo=c.id and a.impto > 0 AND date_format(a.transactiondate , '%Y-%m')='$mes' GROUP BY sorteo ORDER BY sorteo ASC");
         ...
         $query_series_ganadores=mysqli_query($conn, "SELECT numero_premiado_menor, monto pago_premio FROM sorteos_menores_premios  WHERE  sorteos_menores_id=$_sorteo AND premios_menores_id = 2; ");
           $estado='';
                ...
                {  
                  ...
                  $query_pagos_2=mysqli_query($conn, "SELECT date(a.transactiondate) as fecha_pago FROM menor_pagos_detalle a, menor_pagos_recibos b WHERE a.transactioncode= b.transactioncode and a.sorteo=$_sorteo and a.numero=$_numero_ganador_derecho and a.serie=$_numero_serie_ganador_derecho and date_format(a.transactiondate , '%Y-%m')='$mes' ");                
                  ...
                  if ($query_pagos_2)
                  {
                    ...
                    $estado      = 'Pagado';
                    $neto_pagar  = $impto;
                    while ($row_pagos_2 = mysqli_fetch_array($query_pagos_2)) {
                      $_fecha_pago = $row_pagos_2['fecha_pago'];
                    }

                    echo " <tr><td align='center'>".$_sorteo."</td> 
                               <td align='center'>".$_fecha_sorteo."</td> 
                               <td align='center'>".$_fecha_pago."</td> 
                               ...
                               <td align='center'>".number_format($neto_pagar,2,'.',',')."</td></tr>";
...
while ($_numeros_ganadores=mysqli_fetch_array($query_numeros_ganadores))
         ...
         while ($_series_ganadores=mysqli_fetch_array($query_series_ganadores))
                {
                  ...
                  else
                  {
                    $impto_serie=0;
 				          ...
 				          $query_pagos=mysqli_query($conn,"SELECT date(a.transactiondate) as fecha_pago FROM menor_pagos_detalle a, menor_pagos_recibos b WHERE a.transactioncode= b.transactioncode and a.sorteo=$_sorteo and a.numero=$_numero_ganador_derecho and a.serie=$_numero_serie_ganador_derecho and date_format(a.transactiondate , '%Y-%m')='$mes' and a.impto > 0 ");
         				  if ($query_pagos)
         				  {
                     $_fecha_pago_reves ="";
                    if (mysqli_num_rows($query_pagos)>0) 
                    {
                        ...
                        $neto_pagar_serie=$impto_serie;
                         while ($row_pagos_3 = mysqli_fetch_array($query_pagos)) {
                        $_fecha_pago_reves = $row_pagos_3['fecha_pago'];
                         }

                           $neto_serie=$suma_pago_serie-$impto_serie;
                    echo "<tr> <td align='center'>".$_sorteo."</td>
                           <td align='center'>".$_fecha_sorteo."</td>  
                           <td align='center'>".$_fecha_pago_reves."</td>
                             <td align='center'> ".$_numero_ganador_derecho." </td>
                             <td align='center'>".$_numero_serie_ganador_derecho."</td>
                             ...
                             <td align='right'>".$estado."</td>
                             <td align='center'>".number_format($neto_pagar_serie,2,'.',',')."</td></tr>";
                ...
                $neto_pagar_acumulado=$neto_pagar+$sumatoria_Serie;
                        ...
                        echo "<tr><td align='center' colspan='9'> Total a Pagar </td>
                           		  <td align='right'><strong>".number_format($neto_pagar_acumulado,2,'.',',')."</strong></td></tr>";
    ...
    echo "<tr><td align='center' colspan='10'> -- </td></tr><tr><td align='center' colspan='9'> -- </td></tr>
          <tr><td align='center' colspan='9'> Total a Pagar de impuestos generados en el perido de ".$monthName." de ".$año_txt." </td>
