...
<li class="nav-item">
  ...
  <li class="nav-item">
    <a class="nav-link" href="./screen_menor_fecha_pagos_global.php">Pagos de Premios de Lotería Menor</a>
  </li>
</ul>	
<div id="no_print_fr">
<div id="div_wait" class="div_wait">  </div>
<section id="no_print" style="text-align: center; background-color:#ededed; padding-top: 10px;"><h3>Reporte General de Pagos de Premios de Loteria Mayor</h3><br></section>
<section>
<a style = "width:100%" id="non-printable"  class="btn btn-secondary" role="button" data-toggle="collapse" href="#collapse1" aria-expanded="false" aria-controls="collapse"> SELECCIÓN DE PARAMETROS  <i class="far fa-hand-point-down fa-lg"></i></a>
 ...
 <?php     
    ...
    { 
    	 ...
    	 ?>
    	 		<div align="center" id="printOnly" style="margin-top: -20px"><h6><strong>Información General de Pagos de premios <br> Loteria Mayor <br> Del <?php echo $_fecha_inicial; ?> al <?php echo $_fecha_final; ?> </strong></h6>		 </div>
    	       	<div class="table-responsive">
    	       		<table  class="table table-hover table-sm table-bordered">
    	       			<thead><tr align="center">
    	       						<th colspan="2"></th> 
		    	       				<th colspan="2">Pagado</th>  
                                    <th>Pagado Especies</th>
    	       		    ...
    	       		    $query_pagos_fecha=mysqli_query($conn, "SELECT date(a.transactiondate)  fecha_pago, sum(decimos) cantidad_pagada, sum(a.netopayment) neto_pagado ,
                                                                (sum( case when (tipo_premio ='E' ) then a.netopayment else 0 end) )neto_especies,
    	       		    	 									(sum( case when (estado_revision =1 ) then decimos else 0 end) )revisado,
    	       		    ...
    	       		    if ($query_pagos_fecha) 
    	       		    {
                            ...
                            $total_neto_especies             = 0;
                               ...
                               $neto_especies=$row_pagado_fechas['neto_especies'];
    	       		    	   $decimos_revisados=$row_pagado_fechas['revisado'];
    	       		    	   $neto_revisado=$row_pagado_fechas['neto_revisado'];

    	       		    	   ////// debitado
    	       		    	   $query_debitado_banco=mysqli_query($conn, "SELECT monto FROM debitos_banco WHERE DATE(fecha_movimiento)='$fecha_pago' and producto=1 ");
                              ...
                              $query_oracle="SELECT  FECHA_PAGO, SUM(NETO_PAGADO) NETO_PAGADO FROM LOT_PAGOS_DIARIOS WHERE PRODUCTO=1 AND TO_DATE(FECHA_PAGO, 'DD/MM/YY') = TO_DATE('$fecha_oracle', 'DD/MM/YY')  GROUP BY FECHA_PAGO";
                              ...
                              { 
                                ...
                                $decimos_recibido=0;
                                $neto_recibido=0;
    	       		    	   $query_recepcionado= mysqli_query($conn, "SELECT sum(cant_decimos) decimos_recibidos, sum(netopayment) neto_recibido FROM rp_asignacion_agencias_revisor_mayor WHERE date(transactiondate)='$fecha_pago' GROUP BY date(transactiondate)  ");
    	       		    	   ...
    	       		    	   ////// DEBITOS EN PREMIOS
    	       		    	   $query_debitos_total= mysqli_query($conn, "SELECT sum(c.decimos) decimos_debito_pagos, sum(c.neto) neto_debito_pagos FROM mayor_pagos_detalle a, mayor_pagos_recibos b, rp_notas_credito_debito_mayor c WHERE a.transactioncode=b.transactioncode and a.id=c.id_detalle and date(a.transactiondate)='$fecha_pago' and a.estado_revision=2 GROUP BY date(a.transactiondate) ");
    	       		    	   ...
    	       		    	   $query_debitos_parcial= mysqli_query($conn, "SELECT sum(c.decimos) decimos_debito_pagos_parcial, sum(c.neto) neto_debito_pagos_parcial FROM rp_notas_credito_debito_mayor c WHERE c.id_detalle = 0 and date(c.transactiondate)='$fecha_pago' GROUP BY date(c.transactiondate)");
    	       		    	   ...
    	       		    	   $query_faltante= mysqli_query($conn, "SELECT sum(c.decimos) decimos_debito_pagos_parcial, sum(c.netopayment) neto_debito_pagos_parcial FROM rp_faltantes_sobrantes_mayor c WHERE date(c.transactiondate)='$fecha_pago' GROUP BY date(c.transactiondate)");
    	       		    	   if ($query_faltante) 
    	       		    	   {
                                ...
                                $total_neto_especies             = $total_neto_especies           + $neto_especies;    
	    	       		    	...
	    	       		    	$conciliado                      = $neto_pagado-$neto_debitado_banco-$neto_contabilizado-$neto_especies;
                              ...
                              echo "<tr><td>".$fecha_pago."</td>
                                         ...
                                         <td align='right'>".number_format($neto_especies,2)."</td>
    	       		    	...
    	       		    	echo "<tr class='table-success font-weight-bold' >
                                         ...
                                         <td align='right'>".number_format($total_neto_especies,2)."</td>
