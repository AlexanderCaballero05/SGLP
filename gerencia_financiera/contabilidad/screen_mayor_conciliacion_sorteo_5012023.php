...
$billetes_pendientes = 0;
		...
		while ($sorteo_inicial <= $sorteo_final) {
			...
			$query_pagado = mysqli_query($conn, "SELECT sum(decimos) conteo_pagado, sum(a.netopayment) neto_pagado from mayor_pagos_detalle a, mayor_pagos_recibos b where a.transactioncode = b.transactioncode and sorteo=$sorteo_inicial and a.transactionstate in (1,3)");
