...
{ 
  ...
  $query_sorteos_mes=mysqli_query($conn, "SELECT sorteo as id, c.fecha_sorteo FROM mayor_pagos_detalle a, mayor_pagos_recibos b, sorteos_mayores c WHERE a.transactioncode = b.transactioncode AND a.sorteo=c.id and a.imptopayment > 0 AND date_format(a.transactiondate , '%Y-%m')='$mes' GROUP BY sorteo ORDER BY sorteo ASC");

    ?>
		   <table width="100%"  id="table" align="center" class="table table-hover table-bordered table-sm">
		          <thead align='center'> 
		            <tr><td>Tipo Premio</td> 
		            	<td>Sorteo</td> 
  ...
  if (mysqli_num_rows($query_sorteos_mes)>0) {
  	// code...
  ...
  while ( $_row_sorteos=mysqli_fetch_array($query_sorteos_mes)) 
  {
      $_sorteo=$_row_sorteos['id'];   $_fecha_sorteo=$_row_sorteos['fecha_sorteo'];
	   ...
	   $query_numeros_ganadores=mysqli_query($conn, "SELECT descripcion_respaldo, numero_premiado_mayor, monto pago_premio FROM sorteos_mayores_premios where sorteos_mayores_id=$_sorteo and monto>30000 order by monto desc;");
	   	  ...
	   	  if (	mysqli_num_rows($query_numeros_ganadores)>0) 
	   	  {
	   	  			 while ($_numeros_ganadores=mysqli_fetch_array($query_numeros_ganadores))
									    ...
									    $query_archivo_pagos=mysqli_query($conn, "SELECT date(a.transactiondate) as fecha_pago, a.totalpayment total, a.imptopayment impto, a.netopayment neto FROM mayor_pagos_detalle a, mayor_pagos_recibos b where a.transactioncode = b.transactioncode and  a.sorteo=$_sorteo and a.numero=$_numero_ganador");		 

										 if (mysqli_num_rows($query_archivo_pagos)>0) 
										 {
										    while ( $row_nuevo_monto=mysqli_fetch_array($query_archivo_pagos)) 
										    {
										       $_monto_ganador=$row_nuevo_monto['total'];   $estado='Pagado';  $neto_pagar=$row_nuevo_monto['impto']; $fecha_pago_neueva=$row_nuevo_monto['fecha_pago'];
										    }
										 }
										 else  {    $neto_pagar=0;      $estado='No Pagado';  }
										 ...
										 $neto_pagar_acumulado=$neto_pagar+$neto_pagar_acumulado;
										                 ...
										                 echo "<tr><td align='center'>".$_descrip."</td> 
										                           <td align='center'>".$_sorteo."</td> 
										                           <td align='center'>".$_fecha_sorteo."</td> 
										                           <td align='center'>".$fecha_pago_neueva."</td> 
										 ...
										 echo "<tr><td align='center' colspan='10'> -- </td></tr>
											   ...
											   <tr><td align='center' colspan='9'> Total a Pagar de impuestos generados en el perido de ".$mes."</td>
