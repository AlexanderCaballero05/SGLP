...
if (isset($_POST['guardar_nuevo'])) {
	...
	if (mysqli_query($conn, "INSERT INTO rr_hh_mto_preavisos (de_dia, de_mes, de_year, a_dia, a_mes, a_year, dias_preaviso, valor_porcentual_sueldo, tipo_contratacion) VALUES ('$de_dia','$de_mes','$de_year','$a_dia','$a_mes','$a_year','$cantidad_dias','$porcentaje_vacaciones','$id_tipo_contrato') ") === TRUE) {
...
<br>
<h3  align="center" style="color:black;" ><b>MANTENIMIENTO DE PARAMETROS DE PREAVISOS</b></h3>
<br>
</section>
<br>
...
while ($reg_tipo_contrataciones = mysqli_fetch_array($c_tipo_contrataciones)) {
	...
	$c_parametros_en_contrato = mysqli_query($conn, "SELECT * FROM rr_hh_mto_preavisos WHERE estado = 'A' AND tipo_contratacion = '$id_tipo_contratacion' ");
...
while ($reg_parametros_en_contrato = mysqli_fetch_array($c_parametros_en_contrato)) {
		...
		echo "<td>" . $reg_parametros_en_contrato['dias_preaviso'] . "</td>";
...
<div class="input-group" style="margin:5px 0px 5px 0px;">
<div class="input-group-prepend"><span class="input-group-text"># de Dias Preaviso: </span></div>
