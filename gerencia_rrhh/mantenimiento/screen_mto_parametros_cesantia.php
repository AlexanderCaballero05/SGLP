...
if (isset($_POST['guardar_nuevo'])) {
	...
	$maximo_dias = $_POST['maximo_dias'];

	$estado = "A";

	$por_year_trabajado = $_POST['por_year_trabajado'];

	if (mysqli_query($conn, "INSERT INTO rr_hh_mto_cesantias (de_dia, de_mes, de_year, a_dia, a_mes, a_year, dias_cesantia, valor_porcentual_sueldo, tipo_contratacion, base_year_trabajado, maximo_dias) VALUES ('$de_dia','$de_mes','$de_year','$a_dia','$a_mes','$a_year','$cantidad_dias','$porcentaje_vacaciones','$id_tipo_contrato', '$por_year_trabajado', '$maximo_dias') ") === TRUE) {
		echo "<div class = 'alert alert-info'>Parametro de cesantia registrado correctamente.</div>";
	} else {
...
<br>
<h3  align="center" style="color:black;" ><b>MANTENIMIENTO DE PARAMETROS DE CESANTIAS</b></h3>
<br>
</section>
<br>
...
while ($reg_tipo_contrataciones = mysqli_fetch_array($c_tipo_contrataciones)) {
	...
	$c_parametros_en_contrato = mysqli_query($conn, "SELECT * FROM rr_hh_mto_cesantias WHERE estado = 'A' AND tipo_contratacion = '$id_tipo_contratacion' ");
...
while ($reg_parametros_en_contrato = mysqli_fetch_array($c_parametros_en_contrato)) {
		...
		echo "<td>" . $reg_parametros_en_contrato['dias_cesantia'] . "</td>";
		echo "<td>" . $reg_parametros_en_contrato['valor_porcentual_sueldo'] . "</td>";
		echo "<td>" . $reg_parametros_en_contrato['base_year_trabajado'] . "</td>";
		echo "<td>" . $reg_parametros_en_contrato['maximo_dias'] . "</td>";
		echo "</tr>";
	}
	?>
				</tbody>
...
<div class="input-group" style="margin:5px 0px 5px 0px;">
<div class="input-group-prepend"><span class="input-group-text"># de Dias Cesantia: </span></div>
<input type="text" class="form-control" id="cantidad_dias" name="cantidad_dias"  >
<div class="input-group-prepend"><span class="input-group-text">% del Sueldo Prom. Base: </span></div>
<input type="text" class="form-control" id="porcentaje_vacaciones" name="porcentaje_vacaciones" >
</div>

<div class="input-group" style="margin:5px 0px 5px 0px;">
<div class="input-group-prepend"><span class="input-group-text">Por año trabajado: </span></div>
<select name="por_year_trabajado" id="por_year_trabajado" class="form-control" >
	<option value="N">No</option>
	<option value="S">Si</option>
</select>
<div class="input-group-prepend"><span class="input-group-text">Maximo dias: </span></div>
<input type="text" class="form-control" id="maximo_dias" name="maximo_dias" >
</div>
