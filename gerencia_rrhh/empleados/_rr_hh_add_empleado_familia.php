...
?> 
  ...
  .group    { 
    ...
    $("#celularfam").mask("+999-9999-9999-9999", { placeholder: "____-____-_____" });
	}); 
    ...
    function cargar_datos(idn, idenfam, nom, parent, fech, sex,  celtel, dom, idocupa, trab)    
    {    

         // alert(idn +'--' + idenfam+'--' + nom+'--' +parent+'--' +fech+'--' +sex+'--' + celtel+'--' +dom+'--' +idocupa+'--' + trab);
...
<section id="no_print">
    <a style = "width:100%;" id="non-printable"  class="btn btn-secondary" role="button" data-toggle="collapse2" href="" aria-expanded="false" aria-controls="collapse"> AGREGAR NUEVO FAMILIAR <i class="far fa-hand-point-down fa-lg"></i></a>
                ...
                <div class="card-body"  align="center">
                  ...
                  <div class="row">
                       ...
                       <div class="col-sm-6" align="center">                                 
                                  <button class="btn btn-success btn-lg" type="submit" name='addfam' id="addfam" > Agregar Ficha Familiar</button>
                                  <button class="btn btn-primary btn-lg" type="submit" name='updfam' id="updfam" style="display:none;"> Actualizar Ficha Familiar</button>
                      </div>
                      <div class="col-sm-3"></div>
                  </div>                         
              </div>
                  ...
                  <div class="table-responsive">
                    <table id="table_id" class="table table-bordered table-sm table-hove w-auto"   cellspacing="0" style="width:100%;   font-size:13px;">
                       <thead><tr><th></th>                                  
                                  ...
                                  <th >Beneficiario</th>
                                  <th>Ṕorcentaje</th></tr></thead><tbody>
                            <?php 
                            $denti_query     = str_replace( '-', '', $identidad_persona);                           
                            $query_familiares= mysqli_query($conn, "SELECT a.id, a.identidad_familiar,  a.parentesco, b.descripcion parentescotxt, a.nombre_completo, a.fecha_nacimiento, a.celular, a.domicilio, a.ocupaciondesc , ocupacionid,  ocupacion, sexo, a.porcentaje_seguro , lugar_ocupacion FROM rr_hh_empleados_familias a, rr_hh_parentescos b WHERE a.parentesco=b.id and identidad_empleado= '$denti_query' ORDER BY a.id ASC;");
                            ...
                            ?>
                                    ...
                                    <tr onclick = "cargar_datos('<?php echo $denti_query; ?>','<?php echo $identifam; ?>','<?php echo $nombre_completo_tbl ?>','<?php echo $parenteco_tbl_cod; ?>','<?php echo $fecha_nacimiento_tbl; ?>','<?php echo $sexo_tbl; ?>' ,'<?php echo $cel_tbl; ?> ' ,'<?php echo $domicilio_tbl; ?> ' ,'<?php echo $idocupacion; ?> ','<?php echo $trabajo_tbl; ?> '  )">
                            <?php                                  
                                          echo "<td>".$cont."</td>
                                                <td>".$parenteco_tbl."</td>
                                                 <td>".$identifam."</td>                                                 
                                                 ...
                                                 <td>
                                                    ...
                                                    <input type='number'   step='0.01' pattern='^\d*(\.\d{0,2})?$' name='arraybenef[]' style='width:80%' min='0' max='100' required value='".$porcentaje_seguro."' >
                                                    <div>
                                                 </td>
                                                 <td>";
                                                 ?>
...
<?php 
 ...
 {  
    ...
    {        
        ...
        $query_txt_insert_new="INSERT INTO rr_hh_empleados_familias (identidad_empleado, identidad_familiar, fecha_nacimiento, nombre_completo, parentesco,  domicilio, celular,  lugar_ocupacion, ocupacionid, ocupacion, sexo) 
                                                              VALUES('$_ins_identidad_persona', '$_ins_identidad_familiar', '$_ins_fecha_nacimiento', '$_ins_nombre_completo',  '$_ins_parentesco',  '$_ins_domicilio', '$_ins_celular' ,  '$_ins_ocupaciondesc', '$_ins_ocupacionid', '$_ins_ocupaciontxt', '$_ins_sexo');";
          ...
          ?>
            <script type="text/javascript">
              var identity_empleado= "<?php echo $identidad_persona ?>";
                    swal({
                      title: "",
                        text: "Familiar agregado Exitosamente!.",
                        type: "success" 
                      })  
                      .then(function(result){
                          window.location.replace('_rr_hh_add_empleado_familia.php?id='+identity_empleado);
                        });
                  </script>  
          <?php 
        }
    ...
    {            
        ...
        $query_txt_update= "UPDATE rr_hh_empleados_familias 
                            SET nombre_completo = '$_ins_nombre_completo' , parentesco = '$_ins_parentesco', sexo = '$_ins_sexo', domicilio = '$_ins_domicilio' ,  lugar_ocupacion = '$_ins_ocupaciondesc', celular = '$_ins_celular' 
          ...
          echo "<div class='alert alert-success'> <strong> Actualizado con Correctamente </strong></div>";    
            ...
            <script type="text/javascript">
              var identity_empleado= "<?php echo $identidad_persona ?>";
                    swal({
                      title: "",
                        text: "Familiar Actualizado Exitosamente!.",
                        type: "success" 
                      })  
                      .then(function(result){
                          window.location.replace('_rr_hh_add_empleado_familia.php?id='+identity_empleado);
 ...
 <script type="text/javascript">
  ...
  {           
         ...
         if (sum_benef<100) {
              var getbenef2 = "./_msj_validacion_beneficiarios.php?pbe=" + sum_benef+"&op="+1+"&valida="+Math.random(); 
              $('#msg_valid_benef').load(getbenef2); 
         }else{
           var getbenef2 = "./_msj_validacion_beneficiarios.php?pbe=" + sum_benef+"&op="+2+"&valida="+Math.random(); 
              $('#msg_valid_benef').load(getbenef2); 
         }
