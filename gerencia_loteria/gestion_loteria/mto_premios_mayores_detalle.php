...
?>
<i onclick = "asignar_id('<?php echo $id_premio?>','<?php echo $descripcion?>')"  data-toggle="modal" data-target="#myModal" id="drop5" href="#" class="fa fa-plus btn btn-primary" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">
                   </i></td>
<?php
...
if ($update != 0) {
echo "<p align = 'center'><button class = 'btn btn-primary' type = 'submit' name = 'guardar'>Guardar</button>";
echo "<a class = 'btn btn-warning ml-2' type = 'button' href='mto_premios_mayores_detalle_PDF.php?sort=".$id_sorteo."'  target='_blank' role='button' >imprimir</a></p>";
}else{
