...
if (isset($_POST['envio_excel'])) 
{
		...
		$fecha_final = $parametros[2];
		          ...
		          $precio_unitario = $value->precio_unitario* 10;
...
$comision = $value_c->comision;
		...
		$objPHPExcel->getActiveSheet()->SetCellValue('B1','REPORTE DE VENTAS LOTERIA MAYOR (POR USUARIOS) ');
				...
				$i = 4;  
				 $seccionales=mysql_query("SELECT * FROM  fvp_usuarios ");
				  while ( $seccional=mysql_fetch_array($seccionales))
				  {
					...
					$consulta_ventas = mysql_query("SELECT SUM(precio_total) as total FROM fvp_mayor_ventas WHERE  id_sorteo =  '$id_sorteo' AND pani_usuarios_id = '$id_seccional'  and date(fecha_venta) BETWEEN  '$fecha_inicial' and '$fecha_final' ");
		...
		header("Content-Disposition: attachment; filename=\"Reporte_ventas_mayor_usuario.xlsx\"");
