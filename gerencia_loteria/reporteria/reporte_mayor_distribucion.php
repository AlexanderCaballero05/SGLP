...
<br>
<h3  align="center" style="color:black; "  >REPORTE DE DISTRIBUCIONES DE LOTERIA MAYOR (AGENCIAS)</h3>
<br>
</section>
...
if (isset($_POST['seleccionar'])) {
	...
	$sorteos_en_fecha = mysqli_query($conn, "SELECT MIN(id) as sorteo_minimo, MAX(id) as sorteo_maximo FROM sorteos_mayores WHERE YEAR(fecha_sorteo) = '$year'  ORDER BY id ");
	...
	if (mysqli_num_rows($sorteos_en_fecha) > 0) {
		...
		$agencias = mysqli_query($conn, "SELECT DISTINCT(id_seccional) as id_seccional, nombre_seccional, CONCAT(departamento,'/',municipio) as depto_muni  FROM distribucion_mayor_banco WHERE id_sorteo BETWEEN '$min_sorteo' AND '$max_sorteo' GROUP BY id_seccional ORDER BY  id_seccional ASC ");
		...
		$distribuciones_sorteos = mysqli_query($conn, "SELECT id_sorteo ,id_seccional ,SUM(billete_final - billete_inicial + 1) as cantidad  FROM distribucion_mayor_banco WHERE id_sorteo BETWEEN '$min_sorteo' AND '$max_sorteo' GROUP BY id_sorteo , id_seccional ORDER BY id_sorteo, id_seccional ASC ");
...
if (isset($_POST['generar_excel'])) {
	...
	$objPHPExcel->getActiveSheet()->SetCellValue('A1', 'REPORTE DE DISTRIBUCION DE LOTERIA MAYOR (AGENCIAS) ');
