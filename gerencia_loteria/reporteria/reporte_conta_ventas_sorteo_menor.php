...
UNION 
SELECT fecha_venta ,YEAR(fecha_venta) as year , MONTH(fecha_venta) as mes , precio_unitario ,SUM(cantidad) as cantidad,  SUM(total_bruto) as bruto FROM transaccional_ventas WHERE estado_venta = 'APROBADO' AND cod_producto = 3  AND date(fecha_venta) BETWEEN '" . $fecha_i . "' AND '" . $fecha_f . "' GROUP BY YEAR(fecha_venta), MONTH(fecha_venta)
UNION 
SELECT fecha_venta ,YEAR(fecha_venta) as year , MONTH(fecha_venta) as mes , precio_unitario ,SUM(cantidad)/100 as cantidad,  SUM(total_bruto) as bruto FROM transaccional_ventas_general WHERE estado_venta = 'APROBADO' AND cod_producto = 2  AND date(fecha_venta) BETWEEN '" . $fecha_i . "' AND '" . $fecha_f . "' GROUP BY YEAR(fecha_venta), MONTH(fecha_venta)
UNION 
SELECT fecha_venta ,YEAR(fecha_venta) as year , MONTH(fecha_venta) as mes , precio_unitario ,SUM(cantidad)/100 as cantidad,  SUM(total_bruto) as bruto FROM transaccional_ventas WHERE estado_venta = 'APROBADO' AND cod_producto = 2  AND date(fecha_venta) BETWEEN '" . $fecha_i . "' AND '" . $fecha_f . "' GROUP BY YEAR(fecha_venta), MONTH(fecha_venta)
        ...
        echo "<div class = 'card-header bg-primary text-white'><h4 align = 'center'>VENTAS DE LOTERIA MENOR DEL " . $fecha_i . " AL " . $fecha_f . " </h4></div>";
        ...
        while ($reg_conbsulta_banco = mysqli_fetch_array($consulta_banco)) {
          ...
          $consulta_producccion = mysqli_query($conn, "SELECT SUM(series) as producido, precio_unitario FROM sorteos_menores WHERE  YEAR(fecha_sorteo) = '$y' AND MONTH(fecha_sorteo) = '$m' ");
          $ob_produccion = mysqli_fetch_object($consulta_producccion);

          $producido = $ob_produccion->producido;
          $precio_unitario = $ob_produccion->precio_unitario * 100;
          ...
          echo "<td>" . number_format($reg_conbsulta_banco['venta'], 2) . "</td>";
...
<img src="../../assets/firmas_digitales/firma_digital_jefatura_ventas.png" width="350" height="130" >
<br>
