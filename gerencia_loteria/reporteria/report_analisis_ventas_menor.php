...
<br>
<h3 align="center"><b>INFORME DE DISTRIBUCION Y VENTA DE LOTERIA MENOR BOLSAS POR AGENCIAS BANCO DISTRIBUIDOR</b></h3>
<br>
</section>
...
<?php
	...
	$seccionales = mysqli_query($conn, "SELECT SUM(serie_final - serie_inicial + 1) as  cantidad_asignada, nombre_seccional as nombre, id_seccional as cod_seccional , municipio  FROM distribucion_menor_bolsas_banco WHERE id_sorteo = '$id_sorteo'  GROUP BY id_seccional ORDER BY id_seccional ");
	...
	if (mysqli_num_rows($seccionales) > 0) {
		...
		$venta_seccionales = mysqli_query($conn, "SELECT SUM(cantidad) as cantidad_vendida , id_seccional as cod_seccional , MAX(fecha_venta) as f_v  FROM transaccional_ventas_general  WHERE estado_venta = 'APROBADO' AND id_sorteo = '$id_sorteo' AND cod_producto = 3 GROUP BY id_seccional ");
		$i = 0;
...
if (isset($_POST['generar_excel'])) {
	...
	$objPHPExcel->getActiveSheet()->SetCellValue('A1', 'INFORME DE DISTRIBUCION Y VENTA DE LOTERIA MENOR POR AGENCIAS BANCO DISTRIBUIDOR');
	...
	header("Content-Disposition: attachment; filename=\"Reporte_ventas_menor_agencia.xlsx\"");
