...
$sorteos = mysql_query("SELECT * FROM sorteos_mayores WHERE id >= 1195  ORDER BY no_sorteo_may DESC ");
?>
...
<div id="non-printable" class="alert alert-info">
  <h3 align="center">TRANSACCIONES REALIZADAS LOTERIA MAYOR</h3>
...
{
echo '<option value = "'.$row2['id'].'">No.'.$row2['no_sorteo_may'].'</option>' ;
}
?>
</select> 
</td>
...
if ($tipo_consulta == 1) {
$consulta_ventas = oci_parse($conn2, "SELECT SORTEO, CODIGO_FACTURA, FECHA_VENTA, IDENTIDAD, MONTO, NOMBRE_VENDEDOR, EMITIDO, ANULADO, CANTIDAD_BILLETES, NOMBRE_COMPRADOR, FORMA_PAGO FROM LOT_DETALLE_FACTURACION  WHERE SORTEO = $id_sorteo AND LOTERIA = 1 ");
oci_execute($consulta_ventas);                   
}elseif ($tipo_consulta == 2) {
$consulta_ventas = oci_parse($conn2, "SELECT SORTEO, CODIGO_FACTURA, FECHA_VENTA, IDENTIDAD, MONTO, NOMBRE_VENDEDOR, EMITIDO, ANULADO, CANTIDAD_BILLETES, NOMBRE_COMPRADOR, FORMA_PAGO FROM LOT_DETALLE_FACTURACION  WHERE SORTEO = $id_sorteo AND LOTERIA = 1  AND FORMA_PAGO = 1 ");
oci_execute($consulta_ventas);                   
}elseif ($tipo_consulta == 3) {
$consulta_ventas = oci_parse($conn2, "SELECT SORTEO, CODIGO_FACTURA, FECHA_VENTA, IDENTIDAD, MONTO, NOMBRE_VENDEDOR, EMITIDO, ANULADO, CANTIDAD_BILLETES, NOMBRE_COMPRADOR, FORMA_PAGO FROM LOT_DETALLE_FACTURACION  WHERE SORTEO = $id_sorteo AND LOTERIA = 1  AND FORMA_PAGO = 2 ");
oci_execute($consulta_ventas);                   
}elseif ($tipo_consulta == 4) {
$consulta_ventas = oci_parse($conn2, "SELECT SORTEO, CODIGO_FACTURA, FECHA_VENTA, IDENTIDAD, MONTO, NOMBRE_VENDEDOR, EMITIDO, ANULADO, CANTIDAD_BILLETES, NOMBRE_COMPRADOR, FORMA_PAGO FROM LOT_DETALLE_FACTURACION  WHERE SORTEO = $id_sorteo AND LOTERIA = 1  AND FORMA_PAGO = 3 ");
oci_execute($consulta_ventas);                   
}
...
<div class="well">
<h3 align="center">Transacciones de Loteria Mayor</h3>
