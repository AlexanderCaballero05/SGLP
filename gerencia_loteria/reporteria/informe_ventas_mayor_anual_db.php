...
$consulta_sorteos = mysqli_query($conn,"SELECT DISTINCT(a.id_sorteo) as id_sorteo, b.fecha_sorteo  FROM utilidades_perdidas_sorteos as a INNER JOIN sorteos_mayores as b ON a.id_sorteo = b.id WHERE YEAR(b.fecha_sorteo) = '$year' AND a.tipo_loteria = 1 GROUP BY a.id_sorteo ORDER BY a.id_sorteo ASC ");

if ($consulta_sorteos === FALSE) {
...
$consulta_num = mysqli_query($conn,"SELECT numero_premiado_mayor FROM sorteos_mayores_premios WHERE sorteos_mayores_id = '$id_sorteo' AND premios_mayores_id = '1' ");



$ob_numero = mysqli_fetch_object($consulta_num);
$numero_premiado = $ob_numero->numero_premiado_mayor;
