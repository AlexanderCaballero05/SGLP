...
$years = mysqli_query($conn, "SELECT YEAR(fecha_sorteo) as year FROM sorteos_mayores WHERE YEAR(fecha_sorteo) >= '2017'  GROUP BY YEAR(fecha_sorteo) ORDER BY YEAR(fecha_sorteo) DESC ");
...
<h4  align="center" style="color:black; "  id="titulo" >MOVIMIENTO DE VENTAS DE LOTERIA MAYOR
...
////////////////////////////////// EMISION /////////////////////////////////////
		...
		$consulta_emision = mysqli_query($conn, "SELECT SUM(cantidad_numeros) as emision, precio_unitario FROM sorteos_mayores WHERE YEAR(fecha_sorteo) = '$select_year' AND MONTH(fecha_sorteo) = '$i'  ");

		echo mysqli_error($conn);

		$ob_consulta_emision = mysqli_fetch_object($consulta_emision);
		$cantidad_emision = $ob_consulta_emision->emision;
		$precio_unitario = $ob_consulta_emision->precio_unitario * 10;

////////////////////////////////// EMISION /////////////////////////////////////

//////////////////////////////// VENTA BANCO ///////////////////////////////////

		$consulta_venta_banco = mysqli_query($conn, "SELECT SUM(cantidad) as cantidad FROM transaccional_ventas_general WHERE estado_venta = 'APROBADO'  AND cod_producto = 1 AND id_entidad = 3 AND MONTH(fecha_venta) = '$i' AND YEAR(fecha_venta) = '$select_year' ");
...
//////////////////////////////// VENTA FFVPP ///////////////////////////////////
		...
		$consulta_fvp = mysqli_query($conn, "SELECT SUM(cantidad) as cantidad  FROM ( SELECT cantidad FROM transaccional_ventas WHERE estado_venta = 'APROBADO'  AND MONTH(fecha_venta) = '$i' AND YEAR(fecha_venta) = '$select_year' AND cod_producto = 1 UNION ALL SELECT cantidad FROM transaccional_ventas_ajuste WHERE estado_venta = 'APROBADO'  AND MONTH(fecha_venta) = '$i' AND YEAR(fecha_venta) = '$select_year'  AND cod_producto = 1 ) as t   ");
