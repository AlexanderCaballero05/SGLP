...
<br>
<h5  align="center" style="color:black; "  >PATRONATO NACIONAL DE LA INFANCIA <br>REPORTE DE BILLETES NO VENDIDOS POR SORTEO LOTERIA MAYOR </h5>
<br>
...
if (isset($_POST['seleccionar'])) {
    ...
    $c_data_sorteos = mysqli_query($conn, "SELECT a.id, a.fecha_sorteo, a.cantidad_numeros, a.cantidad_numeros - (b.cantidad_banco + c.cantidad_otros) as no_vendido  FROM sorteos_mayores as a INNER JOIN (SELECT id_sorteo ,SUM(cantidad) as cantidad_banco FROM transaccional_ventas_general WHERE id_sorteo BETWEEN  '$sorteo1' AND '$sorteo2'  AND estado_venta = 'APROBADO' AND cod_producto = 1 GROUP BY id_sorteo  )  as b INNER JOIN (SELECT id_sorteo ,SUM(cantidad) as cantidad_otros FROM transaccional_ventas WHERE id_sorteo BETWEEN  '$sorteo1' AND '$sorteo2'  AND estado_venta = 'APROBADO' AND cod_producto = 1 GROUP BY id_sorteo  )  as c ON a.id = b.id_sorteo AND a.id = c.id_sorteo  WHERE a.id BETWEEN '$sorteo1' AND '$sorteo2' GROUP BY a.id ");
...
<table class="table table-bordered">
    ...
    while ($reg_data_sorteos = mysqli_fetch_array($c_data_sorteos)) {
        ...
        echo "<td>".number_format($reg_data_sorteos['no_vendido'])."</td>";
        echo "</tr>";
