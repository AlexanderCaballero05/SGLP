...
require './cc_envio_produccion_menor_db.php';
...
<br>
<h2  align="center" style="color:black;"  >CONTROL DE REPOSICIONES LOTERIA MENOR</h2>
<br>
</section>
...
if (isset($_POST['seleccionar'])) {
	...
	$consulta_detalle = mysqli_query($conn, "SELECT a.numero as num_lista ,a.id_revisor, a.serie_inicial, a.serie_final , a.serie_final -  a.serie_inicial + 1 as cantidad  , b.nombre_completo, (select count(id) from cc_revisores_sorteos_menores_control as c where c.id_sorteo = a.id_sorteo AND c.id_revisor = a.id_revisor AND numero_revision = 2 AND especial = 'NO') as conteo_reposicion , (select count(id) from cc_revisores_sorteos_menores_control as d where d.id_sorteo = a.id_sorteo AND d.id_revisor = a.id_revisor AND especial = 'SI' AND numero_revision = 2) as conteo_reposicion_re  FROM cc_revisores_sorteos_menores as a INNER JOIN pani_usuarios as b ON a.id_revisor = b.id WHERE a.id_sorteo = '$id_sorteo' ");
	...
	while ($reg_consulta_detalle = mysqli_fetch_array($consulta_detalle)) {
		...
		echo "<td>" . $reg_consulta_detalle['serie_inicial'] . "</td>";
		echo "<td>" . $reg_consulta_detalle['serie_final'] . "</td>";
	...
	$consulta_fin = mysqli_query($conn, "SELECT * FROM cc_produccion_menor WHERE id_sorteo = '$id_sorteo' AND numero_revision = '1' LIMIT 1 ");
...
/////////////////////////////////////////////////////////////////////////////////////
	...
	$revisiones_realizadas = mysqli_query($conn, "SELECT numero_revision  FROM cc_revisores_sorteos_menores_control WHERE id_sorteo = '$id_sorteo' AND estado != 'APROBADO' GROUP BY numero_revision ORDER BY numero_revision ASC ");
	...
	while ($reg_revisiones = mysqli_fetch_array($revisiones_realizadas)) {
		...
		$consulta_detalle = mysqli_query($conn, "SELECT a.numero as num_lista ,a.id_revisor, a.serie_inicial, a.serie_final , a.serie_final -  a.serie_inicial + 1 as cantidad  , b.nombre_completo, (select count(id) from cc_revisores_sorteos_menores_control as c where c.id_sorteo = a.id_sorteo AND c.id_revisor = a.id_revisor AND numero_revision = '$numero_revision' AND estado = 'REPROBADO' AND especial = 'NO') as conteo_reposicion , (select count(id) from cc_revisores_sorteos_menores_control as d where d.id_sorteo = a.id_sorteo AND d.id_revisor = a.id_revisor AND especial = 'SI' AND numero_revision  = '$numero_revision' AND estado = 'REPROBADO') as conteo_reposicion_re  , (select count(id) from cc_revisores_sorteos_menores_control as d where d.id_sorteo = a.id_sorteo AND d.id_revisor = a.id_revisor  AND numero_revision  = '$numero_revision' AND estado = 'PENDIENTE') as  conteo_pendiente   FROM cc_revisores_sorteos_menores as a INNER JOIN pani_usuarios as b ON a.id_revisor = b.id WHERE a.id_sorteo = '$id_sorteo' ");
		...
		$consulta_fin = mysqli_query($conn, "SELECT * FROM cc_produccion_menor WHERE id_sorteo = '$id_sorteo' AND numero_revision = '$numero_revision' LIMIT 1 ");
