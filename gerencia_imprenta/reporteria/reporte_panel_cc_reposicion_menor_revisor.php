...
<br>
<h2  align="center" style="color:black; "  >REPOSICIONES DE LOTERIA MENOR POR REVISOR</h2>
<br>
...
<option value="0">Serie y decena</option>
<option value="1">Decena y serie</option>
</select>
...
if (isset($_POST['seleccionar'])) {
	...
	$revisores = mysqli_query($conn, "SELECT a.id_revisor,a.num_lista, b.nombre_completo FROM cc_revisores_sorteos_menores_control as a INNER JOIN pani_usuarios as b ON a.id_revisor = b.id WHERE a.id_sorteo = '$id_sorteo'  GROUP BY a.id_revisor, a.num_lista ORDER BY a.num_lista ");
...
Revisor ' . $revisor['nombre_completo'] . '<br>  Asignacion ' . $num_lista . '
		...
		$numero_revisiones = mysqli_query($conn, "SELECT * FROM cc_revisores_sorteos_menores_control as a  WHERE a.id_sorteo = '$id_sorteo' AND num_lista = '$num_lista'  GROUP BY a.numero_revision  ");

		while ($revision = mysqli_fetch_array($numero_revisiones)) {

			$num_rev = $revision['numero_revision'];
			$parametros = $id_sorteo . '_' . $reporte_revision . '_' . $num_rev . '_' . $num_lista . '_' . $orden;
			$num_rev = $num_rev - 1;

			echo '
<a style = "width:100%; margin-top: 2px; margin-bottom: 2px;"  class="btn btn-success" target = "_blanck" href = "./reporte_panel_cc_reposicion_menor_revisor_detalle.php?par=' . $parametros . '" >
