...
if (isset($_POST['seleccionar'])) {
	...
	$parametros_menor = mysqli_query($conn, "SELECT * FROM sorteos_menores_produccion where id_sorteo = '$id_sorteo' ");

	$i = 1;
	while ($reg = mysqli_fetch_array($parametros_menor)) {
		$v_salto[$i] = $reg['salto'];
		$i++;
	}
	...
	if ($fecha != '') {
		...
		$controles_iniciados = mysqli_query($conn, "SELECT a.contador_final,a.billetes_buenos,a.id,a.fecha,a.id_orden,a.contador_final_maquina,a.etapa,a.maquina,a.contador_inicial,a.jornada FROM pro_control_menor as a  WHERE a.id_orden = '$id_sorteo' AND a.fecha  = '$fecha' AND a.estado = 'FINALIZADO' ORDER BY a.fecha ,a.hora_inicial ,a.etapa , a.contador_final, a.maquina ASC");
		...
		$controles_iniciados = mysqli_query($conn, "SELECT a.contador_final,a.billetes_buenos,a.id,a.fecha,a.id_orden,a.contador_final_maquina,a.etapa,a.maquina,a.contador_inicial,a.jornada FROM pro_control_menor as a  WHERE a.id_orden = '$id_sorteo' AND a.estado = 'FINALIZADO' ORDER BY a.fecha ,a.hora_inicial ,a.etapa , a.contador_final, a.maquina ASC ");
	...
	while ($control_iniciado = mysqli_fetch_array($controles_iniciados)) {
		...
		$max_min_contador = mysqli_query($conn, "SELECT fecha, etapa ,jornada,  billetes_buenos,  hora_final, hora_inicial, contador_inicial as minimo,maquina , contador_final_maquina as maximo, grupo FROM pro_control_menor WHERE id_orden = '$id_sorteo' AND id = '$id_detalle_control'  ");
		...
		$grupo = $ob_maximo_menor->grupo;
		$jornada = $ob_maximo_menor->jornada;

		$c_prensistas = mysqli_query($conn, "SELECT * FROM pro_control_prensistas_menor WHERE id_control = '$id_detalle_control' ");
...
<div class="card-header">
  <h4 style = "text-align: center" >CONTROL DE PRODUCCION MENOR</h4>
</div>
<div class="card-body">
...
<td></td>
		...
		$detalle = mysqli_query($conn, "SELECT * FROM pro_control_detalle_menor WHERE id_control = '$id_detalle_control' ");
		if ($detalle === false) {
			echo mysqli_error();
		}
