...
if (isset($_POST['guardar_detalle'])) {
	...
	$grupo = $_POST['grupo'];
	$v_grupo = explode('/', $grupo);
	$grupo_billetes = $v_grupo[0];
	$id_control = $_POST['guardar_detalle'];
	$cantidad = $_POST['cantidad'];
	$cantidad_i = $_POST['cantidad_impresiones'];

	if ($cantidad > 0) {
		$cantidad = $cantidad * $cantidad_i;
	} else {
		$cantidad = $cantidad_i;
	}
	...
	if (mysqli_query($conn, "INSERT INTO pro_control_detalle (id_control,grupo,cantidad,tipo,de_billete,a_billete,de_registro,a_registro,numeradora_parcial,observaciones) VALUES ('$id_control','$grupo_billetes','$cantidad','$tipo','$de_billete','$a_billete','$de_registro','$a_registro','$numeradora_parcial','$observaciones') ") === false) {
		echo mysqli_error($conn);
	} else {
