...
if (isset($id_control)) {
	...
	$info_control = mysqli_query($conn, "SELECT a.estado,a.contador_final_maquina,a.id,a.fecha,a.etapa,a.id_orden,a.id_orden_2,a.maquina,a.contador_inicial,a.jornada,a.billete_inicial,a.billete_final,a.hora_inicial,a.hora_final,a.billetes_buenos,a.id_operador_encargado,a.contador_final,b.maquina as nombre_maquina, c.no_sorteo_may,c.desde_registro,c.patron_salto FROM pro_control as a INNER JOIN pro_maquinas as b INNER JOIN
	...
	$busqueda_listas = mysqli_query($conn, "SELECT * FROM cc_revisores_sorteos_mayores WHERE id_sorteo = '$id_sorteo' GROUP BY numero ORDER BY numero ASC ");
	$busqueda_revisiones = mysqli_query($conn, "SELECT * FROM cc_revisores_sorteos_mayores_control WHERE id_sorteo = '$id_sorteo' GROUP BY numero_revision ORDER BY numero_revision ASC ");
...
if (isset($_POST['guardar_detalle'])) {
	...
	if (mysqli_query($conn, "INSERT INTO pro_control_detalle (id_control,lista,revision,cantidad,tipo,numeradora_parcial,observaciones) VALUES ('$id_control','$lista','$revision','$cantidad','$tipo','$numeradora_parcial','$observaciones') ") === false) {
		echo mysqli_error();
	} else {
